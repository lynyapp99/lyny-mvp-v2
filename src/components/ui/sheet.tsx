...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 touch-manipulation",
      className,
    )}
    onClick={() => {
      ...
      >
        ...
        {(side === "left" || side === "right") && (
          <SheetPrimitive.Close 
            className="absolute right-4 top-4 z-[102] rounded-full p-2 min-w-[44px] min-h-[44px] flex items-center justify-center bg-background/80 backdrop-blur-sm hover:bg-muted shadow-md transition-all duration-150 active:scale-95 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 disabled:pointer-events-none touch-manipulation"
            onClick={() => {
              if ("vibrate" in navigator) navigator.vibrate(10);
            }}
